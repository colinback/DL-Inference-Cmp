...
/*
 * Multidimensional float array that allows sharing of the underlying
 ...
 * another in C/C++, such as from float[3][8] to float[2][3][4].
 */
public class Cfloat {
  ...
  /*
   * Constructor with supplied float[] as the data array, starting
   * at the offset 0, having the same size as f.
   */
  public Cfloat(float[] f) {
    this(new int[]{f.length}, f, 0);
  }

  /*
   * Constructor with supplied float[] as the data array, starting
   * at the offset 0. Note d does not need to map the entire f.
   */
  public Cfloat(int[] d, float[] f) {
    this(d, f, 0);
  }

  /*
   * Constructor with supplied float[] as the data array, starting
   * at the offset n into f. Note d does not need to map the entire f.
   */
  public Cfloat(int[] d, float[] f, int n) {
    if (n < 0)
  ...
  /*
   * Constructor sharing underlying data array with another Cfloat, starting
   * at the offset 0, having the same dimensions as f.
   */
  ...
  /*
   * Constructor sharing underlying data array with another Cfloat, starting
   * at the offset 0. Note d and f can be any arbitrary dimensions and
   * d does not need to map the entire f.
   */
  public Cfloat(int[] d, Cfloat f) {
    this(d, f, 0);
  }

  /*
   * Constructor sharing underlying data array with another Cfloat, starting
   * at the offset n into f. Note d and f can be any arbitrary dimensions and
   * d does not need to map the entire f.
   */
  public Cfloat(int[] d, Cfloat f, int n) {
    if (n < 0)
      throw new IllegalArgumentException("offset is negative");
    dims = d.length == 0 ? new int[]{1} : d;
    size = size(dims);
   ...
   */
  public void set(int[] coord, float f) {
    data[idx(coord)] = f;
  }
  ...
  /*
   * Share the underlying data array with another float[], starting
   * at the offset 0.
   */
  public void share(float[] f) {
    share(f, 0);
  }

  /*
   * Share the underlying data array with another float[], starting
   * at the offset n into f.
   */
  public void share(float[] f, int n) {
    if (n + size < 0)
      throw new IllegalArgumentException("offset + array size overflow");
    if (n + size > f.length)
      throw new IllegalArgumentException("offset + array size out of bound");
  ...
  /*
   * Share the underlying data array with another Cfloat, starting
   * at the offset 0.
   */
  public void share(Cfloat f) {
    share(f, 0);
  }
   ...
   * at the offset n into f.
   */
  public void share(Cfloat f, int n) {
    if (n + size < 0)
      throw new IllegalArgumentException("offset + array size overflow");
    if (n + size > f.size())
      throw new IllegalArgumentException("offset + array size out of bound");

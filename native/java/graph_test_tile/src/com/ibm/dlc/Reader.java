...
public class Reader {
  ...
  // Read binary file with specified endian into Cfloat data array.
  // If jar == true, path refers to a file inside a jar file.
  public static void Read_float(Cfloat obj, String path, ByteOrder endian, boolean jar) throws Exception {
    final InputStream is = jar ? GetJarFileInputStream(path) :
                                 new FileInputStream(path);
    final ByteArrayOutputStream baos = new ByteArrayOutputStream();
    final byte[] buf = new byte[Math.min(Integer.MAX_VALUE, is.available())];
    ...
    final FloatBuffer fb = ByteBuffer.wrap(baos.toByteArray())
                                     .order(endian)
                                     .asFloatBuffer();
    final float[] floats = obj.raw();
    final int size = obj.size();
    if (fb.limit() != size) {
      throw new RuntimeException(
        path + " size " + fb.limit() + " != " + "Cfloat size " + size);
    }
    fb.get(floats);
  }
  ...
  // Read binary file with specified endian into Cint32_t data array.
  // If jar == true, path refers to a file inside a jar file.
  public static void Read_int32_t(Cint32_t obj, String path, ByteOrder endian, Boolean jar) throws Exception {
    final InputStream is = jar ? GetJarFileInputStream(path) :
                                 new FileInputStream(path);
    final ByteArrayOutputStream baos = new ByteArrayOutputStream();
    final byte[] buf = new byte[Math.min(Integer.MAX_VALUE, is.available())];
    ...
    final IntBuffer ib = ByteBuffer.wrap(baos.toByteArray())
                                   .order(endian)
                                   .asIntBuffer();
    final int[] ints = obj.raw();
    final int size = obj.size();
    if (ib.limit() != size) {
      throw new RuntimeException(
        path + " size " + ib.limit() + " != " + "Cint32_t size " + size);
    }
    ib.get(ints);
  }
  ...
  // Read binary file with specified endian into Cint64_t data array.
  // If jar == true, path refers to a file inside a jar file.
  public static void Read_int64_t(Cint64_t obj, String path, ByteOrder endian, Boolean jar) throws Exception {
    final InputStream is = jar ? GetJarFileInputStream(path) :
                                 new FileInputStream(path);
    final ByteArrayOutputStream baos = new ByteArrayOutputStream();
    final byte[] buf = new byte[Math.min(Integer.MAX_VALUE, is.available())];
    ...
    final LongBuffer lb = ByteBuffer.wrap(baos.toByteArray())
                                    .order(endian)
                                    .asLongBuffer();
    final long[] longs = obj.raw();
    final int size = obj.size();
    if (lb.limit() != size) {
      throw new RuntimeException(
        path + " size " + lb.limit() + " != " + "Cint64_t size " + size);
    }
    lb.get(longs);
  }
  ...
  // Read binary file with specified endian into Cbool data array.
  // If jar == true, path refers to a file inside a jar file.
  // Note that endian argument is ignored but kept for consistency.
  public static void Read_bool(Cbool obj, String path, ByteOrder endian, Boolean jar) throws Exception {
    final InputStream is = jar ? GetJarFileInputStream(path) :
                                 new FileInputStream(path);
    final byte[] buf = obj.raw();
    int l = obj.size(), m = 0;
    for (int n = 0; (n = is.read(buf, n, l-n)) > 0; m += n);
    is.close();
    if (m != l) {
      throw new RuntimeException(path + " size " + m + " != " + "Cbool size " + l);
    }
  }
}
